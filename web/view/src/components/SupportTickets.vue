    ...
    <div  v-if="gdata.isLogin"  class="tab-pane in active" id="app-about">
        ...
        <div style="clear:both;float:left;">
            <form class="form-inline" role="form">
      			  <router-link style="width:120px;" tag="button" :to="{name:'newticket'}" class="btn-p btn btn-primary pull-left">发起提问</router-link>
             </form>
        </div>
          ...
          <table class="table" >
            ...
            <tr v-for="v in list.data">
                ...
                <td>{{stateText[v.state]}}</td>
                   <td>{{v.last_reply}}</td>
                <td>
                      <router-link tag="button" :to="{name:'modifyticket', params:{id:v.id}}"  type="button" style="font-size:13px;height:35px;" class="btn-p btn btn-primary btn-sm">查看</router-link>
    ...
    export default{
        ...
        methods: {
        	loadList(ps, pn){
        		...
        		this.$http.get('/tickets?ps='+ps+'&pn='+pn).then((response)=>{
