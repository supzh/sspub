        ...
        <div style="float:left;margin-bottom:60px;">
         ...
         <div style="clear:both;float:left;">
            <form class="form-inline" role="form">
      			  <router-link style="width:auto;" tag="button" :to="{name:'newserver'}" class="btn-p btn btn-primary pull-left">新增服务器</router-link>
             </form>
        </div>
        ...
        <div style="float:left;clear:both; width:100%;margin-top:30px;border-radius:4px!important;border:1px solid #edf0f2;">
          <table class="table" >
            ...
            <tr v-for="v in list.data">
               ...
               <td>{{v.max_bandwidth_mo}}</td>
               <td>{{v.used_bandwidth_mo}}</td>
                <td>
                      <router-link tag="button" :to="{name:'modifyserver', params:{id:v.id}}"  type="button" style="font-size:13px;height:35px;" class="btn-p btn btn-primary btn-sm">修改</router-link>
        ...
        methods: {        	
        	...
        	del(v){        		        		
            	...
            	swal({
            		...
            		function(){
            			vc.$http.get('/admin/deleteserver/'+v.id).then((response)=>{
        	...
        	loadList(ps, pn){
        		...
        		this.$http.get('/admin/servers?ps='+ps+'&pn='+pn).then((response)=>{        			  
