    ...
    <div class="tab-pane in active" id="app-about">
        <ol class="breadcrumb">
          ...
          <div style="float:left;margin-bottom:60px;">
	        <ol class="submenu">
	            <li :class="{active:this.$route.name=='faq'}" @click="gosupport">知识库</li>
             	...
             	<form class="form-horizontal" role="form">
						<div class="col-sm-12" style="padding:10px;font-size:20px;margin-left:0;padding-left:0;font-size:24px;">
                         ...
                         <div class="col-sm-12" style="padding:0;padding-bottom:10px;">
								...
								<li v-for="v in list" @click="godoc(v)"><div style="float:left;color:#1e88e5;font-size:23px;padding-right:15px;">+</div> {{v.title}}</li>
						 </ul>
...
<script>
    export default{
        ...
        methods: {
        	godoc(v){
        		this.$router.push({name:'doc', params:{title:v.title}})
        	},
        	loadList(){
        		this.$http.get('/docsbycategory?name='+this.$route.params.name).then((response)=>{
                      if(response.data.code == 0 && response.data.data != undefined) {
                         this.list = response.data.data
                         if(this.list.length!=0) {
                         	this.categoryName = this.showcategoryName = this.$route.params.name
                         } else {
                        	  this.showcategoryName = '找不到相关内容。'
                         }
                      } else {
                    	  this.showcategoryName = '检索内容失败。'
                      }
        		})
        	},
        	gosupport(){
